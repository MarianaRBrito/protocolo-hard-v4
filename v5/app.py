...
from etapas_heuristicas import executar_etapas_heuristicas
...
def parse_jogo(s):
    try:
        ns = [int(x) for x in s.strip().replace(",", " ").split()]
...
def parse_fixas(txt):
    vals = sorted(set(int(x) for x in txt.replace(",", " ").split()))
    if any(x < 1 or x > 25 for x in vals):
        raise ValueError("Há dezenas fora do intervalo 01-25.")
    if len(vals) > 10:
        raise ValueError("Fixas demais travam a carteira. Use no máximo 10.")
    return vals
...
with st.sidebar:
    ...
    offset_ref = st.number_input(
        "Referência (0=último, 1=penúltimo)",
        min_value=0,
        max_value=n_conc - 1,
        value=0,
        step=1,
    )
    ref_idx = n_conc - 1 - offset_ref
    threshold = st.slider("Threshold N+1/N+2 (%)", 50, 100, 80)

    h = get_param_hash(janela, offset_ref, threshold)
    ...
    st.progress(
        conc / total if total > 0 else 0,
        text=f"{conc}/{total} etapas core"
    )

    st.markdown("---")
    if st.button("⚡ EXECUTAR PROTOCOLO COMPLETO", type="primary", use_container_width=True, key="btn_exec_sidebar"):
    ...
    if st.button("🔄 Resetar pipeline", use_container_width=True, key="btn_reset_sidebar"):
        resetar_pipeline()
        st.rerun()

    st.markdown("---")
    st.subheader("🎰 Backtesting manual")
    j1 = st.text_input("Jogo 1 (15 números separados por espaço)", key="bt_j1")
    j2 = st.text_input("Jogo 2", key="bt_j2")
...
with tabs[0]:
    ...
    if st.button("▶️ Executar Térmica", type="primary", key="btn_termica"):
    ...
    if not t_res:
        ...
        with col_f:
            ...
            rows_f = [
                {"Faixa": k, "Freq%": round(v["media_freq"] * 100, 1)}
        ...
        with col_q:
            ...
            rows_q = [
                {"Quadrante": k, "Freq%": round(v["media_freq"] * 100, 1)}
...
with tabs[1]:
    ...
    if st.button("▶️ Executar Atraso", type="primary", disabled=not ex.concluido("frequencia_simples"), key="btn_atraso"):
...
with tabs[2]:
    ...
    with col_e:
        if st.button("▶️ Executar Estatísticas", type="primary", disabled=not ex.concluido("atraso_atual"), key="btn_estatisticas"):
            with st.spinner("Calculando..."):
                executar_etapas_estatisticas(ex, sorteios, janela, ref_idx, threshold)
            st.rerun()
    with col_h:
        if st.button("▶️ Executar Heurísticas", type="secondary", disabled=not ex.concluido("backtesting"), key="btn_heuristicas"):
            with st.spinner("Calculando..."):
    ...
    if not ex.concluido("qui_quadrado"):
        ...
        st.dataframe(
            pd.DataFrame(
                [{"Rank": i + 1, "Dezena": d, "Score": round(sc[d], 4)} for i, d in enumerate(ranking)]
            ),
            hide_index=True,
        ...
        with col_a:
            ...
            if tr:
                if tr.get("altas"):
                    st.success(f"🔼 Alta: {' '.join(f'{d:02d}' for d in tr['altas'])}")
                if tr.get("baixas"):
                    st.warning(f"🔽 Baixa: {' '.join(f'{d:02d}' for d in tr['baixas'])}")
        with col_b:
...
with tabs[3]:
    ...
    with col_f1:
        filtro_grupo = st.selectbox("Grupo", ["Todos"] + sorted(df_status["Grupo"].unique().tolist()), key="filtro_grupo")
    with col_f2:
        filtro_status = st.selectbox("Status", ["Todos", "concluido", "pendente", "erro"], key="filtro_status")
    with col_f3:
        mostrar_exp = st.checkbox("Mostrar experimentais", value=False, key="mostrar_exp")
    ...
    def color_status(val):
        if val == "concluido":
            return "background-color:#0d3320;color:#00FF88"
        if val == "erro":
    ...
    c3.metric(
        "Steps ativos",
        len([s for s in STEP_REGISTRY if ex.concluido(s["id"]) and s["influencia_gerador"]])
    )
...
with tabs[4]:
    ...
    conc, total = ex.progresso(False)
    pipeline_ok = ex.todas_core_ok()

    if not pipeline_ok:
        faltando = [
            s["id"]
            for s in STEP_REGISTRY
            if not s.get("experimental") and not ex.concluido(s["id"])
        ]
        st.warning(f"Etapas obrigatórias pendentes para gerar: {', '.join(faltando)}")

        if st.button("⚡ Executar pipeline completo", type="primary", key="btn_exec_gerar"):
        ...
        with col_g1:
            n_jogos = st.number_input("Quantos jogos?", 1, 20, 5, key="n_jogos")
            perfil = st.selectbox("Perfil", ["Híbrida", "Agressiva", "Equilibrada"], key="perfil")
        with col_g2:
            seed = st.number_input("Seed (0=aleatório)", 0, 99999, 0, key="seed")
            max_comum = st.slider("Máx. em comum entre jogos", 5, 14, 11, key="max_comum")
        with col_g3:
            fixas_inp = st.text_input("Dezenas fixas (ex: 05 13 22)", key="fixas_inp")
            n_cand = st.slider("Candidatos por estratégia", 20, 150, 60, key="n_cand")

        fixas = []
        if fixas_inp.strip():
            try:
                fixas = parse_fixas(fixas_inp)
            except Exception as e:
                st.error(f"Erro nas fixas: {e}")
                st.stop()

        st.info(f"💰 {n_jogos} × R$3,50 = **R${n_jogos * 3.5:.2f}**")

        if st.button("🎯 GERAR COMBINAÇÕES", type="primary", use_container_width=True, key="btn_gerar_combinacoes"):
        ...
        if res and isinstance(res, dict) and res.get("jogos_finais"):
            ...
            st.success(f"✅ {len(jogos_finais)} jogo(s) | {res.get('n_candidatos', len(jogos_finais))} candidatos")
            if vencidas_g:
                st.error(f"🔴 Vencidas: {' '.join(f'{d:02d}' for d in vencidas_g)}")
            ...
            for i, av in enumerate(jogos_finais):
                jogo = av["jogo"]
                with st.expander(
                    f"🃏 J{i + 1} | Score {av['score_total']} | Soma {av['soma']} | 11+:{av['pct_11']}%",
                    expanded=(i == 0)
                ):
                    html = "".join(
                        f'<span class="dez-box {"dez-venc" if d in vencidas_g else "dez-hot" if d in fortes_n12 else ""}">{d:02d}</span>'
                        for d in jogo
                    )
            ...
            if len(jogos_finais) > 1 and "hamming" in res:
...
with tabs[5]:
    ...
    with col_r2:
        ...
        if at:
            if at.get("vencidas"):
                st.error(f"🔴 {' '.join(f'{d:02d}' for d in at['vencidas'])}")
            if at.get("atrasadas"):
                st.warning(f"🟠 {' '.join(f'{d:02d}' for d in at['atrasadas'])}")
            if at.get("no_ciclo"):
                st.info(f"🟡 {' '.join(f'{d:02d}' for d in at['no_ciclo'])}")
        else:
            st.info("Não executada.")
    ...
    if any(v > 0 for v in sc.values()):
        ...
        with col_sc1:
            ...
            for i, d in enumerate(ranking[:13]):
                tags = ""
                if at and d in at.get("vencidas", []):
                    tags += " 🔴"
                if at and d in at.get("atrasadas", []):
                    tags += " 🟠"
                if d in n12.get("fortes", []):
                    tags += " 🔥"
    ...
    if res and isinstance(res, dict) and res.get("jogos_finais"):
        ...
        for i, av in enumerate(res["jogos_finais"]):
            ...
            st.write(
                ...
                f"Score:{av['score_total']} | Soma:{av['soma']} | 11+:{av['pct_11']}% "
                f"{'✅' if not erros else '⚠️'}"
            )
...
with tabs[6]:
    ...
    if not jogos_bt:
        ...
        for i, jogo in enumerate(jogos_bt):
            ...
            st.write(
                f"**Soma:** {sum(jogo)} | "
                f"**Pares:** {sum(1 for n in jogo if n % 2 == 0)} | "
                f"**Primos:** {len(set(jogo) & PRIMOS)} | "
                f"**Fibonacci:** {len(set(jogo) & FIBONACCI)} | "
                f"**Múltiplos de 3:** {len(set(jogo) & MULTIPLOS_3)}"
            )
